 ...
 */
public class RecommendFriendsV {
    ...
    public int recommendFriends(int[][] friends, int user) {
        ...
        Set<Integer> myFriendSet = new HashSet<>(myFriends.length);
        ...
        for (int i = 0; i < friends.length; i++) {
            if (myFriendSet.contains(i) || friends[i].length <= max) {
                continue;
            }
            count = 0;
            for (int fi : friends[i]) {
                if (myFriendSet.contains(fi)) {
                    ++count;
                }
            }
            if (count > max) {

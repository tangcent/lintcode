 ...
 */
public class RecommendFriendsIII {
    ...
    public int recommendFriends(int[][] friends, int user) {
        ...
        IntSet visitedFriendSet = new IntSet();
        visitedFriendSet.add(user);

        for (int myFriend : myFriends) {
            myFriendSet.add(myFriend);
            visitedFriendSet.add(myFriend);
        }
        ...
        for (int myFriend : myFriends) {
            for (int i : friends[myFriend]) {
                if (visitedFriendSet.tryAddFailed(i)) {
                    continue;
                }
